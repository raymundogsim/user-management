...
const handleRegions =  () => {
  ...
  if(profile.regCode){
    handleArea('regCode', profile)
  }
  if(profile.provCode){
    handleArea('provCode', profile)
  }
  if(profile.citymunCode){
    handleArea('citymunCode', profile)
  }
}
...
useEffect(() => {
  if(!profile){
    setContactData({areaCode: user.userLevel, regCode: area.regCode, provCode: area.provCode, citymunCode: area.citymunCode, brgyCode: area.brgyCode})
  } else {
    setContactData({...profile, userLevel: user.userLevel, areaCode: user.areaCode});
    }
  setRnd(Math.random())
}, [user])

console.log(profile,user, area,'Profile')
  return (
    <Box m="10px">
